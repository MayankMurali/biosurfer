...
def _process_gene(
    gene_name: str, db: 'Database', flog, gencode: bool, gene_to_anchor_tx: dict[str, str],
    gene_to_gc_transcripts: dict[str, list[str]], gene_to_pb_transcripts: dict[str, list[str]]
):
    """Compute cblock/pblock records for every alternative isoform of one gene against
    its chosen anchor isoform. Extracted from process_chr's per-gene loop so it takes
    its dependencies as explicit parameters instead of closing over process_chr's
    locals."""
    out = []
...
def process_chr(chr: str, db_name: str, log_file: 'Path', gencode: bool, gene_to_anchor_tx: dict[str, str]):
    ...
    with open(log_file, 'w') as flog:
        process_gene = partial(
            _process_gene, db=db, flog=flog, gencode=gencode, gene_to_anchor_tx=gene_to_anchor_tx,
            gene_to_gc_transcripts=gene_to_gc_transcripts, gene_to_pb_transcripts=gene_to_pb_transcripts
        )
